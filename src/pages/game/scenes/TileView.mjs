...
function displayImageInHTML({ htmlElementId, imageKey, scene } = {}) {
  ...
  if (
    texture instanceof Phaser.Textures.Texture &&
    imgElement instanceof Element
  ) {
...
export default {
  ...
  create({ hex }) {
    ...
    if (hex.tile.improvement) {
      ...
      graphics.beginPath();
      const [firstCorner, ...otherCorners] = tileHex.corners.map((point) =>
      ...
      const img = this.textures
        .get(`improvements.${hex.tile.improvement.key}`)
        .getSourceImage();
